...
<html lang="es" dir="ltr">
  <head>
    <meta charset="utf-8">
    <title>Selecciona un profesor</title>
        ...
        if($count > 0){
          while($rows = mysqli_fetch_assoc($resultados)){
            echo "<div class='row' style='margin-top: 1%;'>
                <div class='col'>
                    <h1 class='text-left'>".$rows['email']."</h1>
                    <h1 class='text-left'>".$rows['nombre']."</h1>
                    <h1 class='text-left'>".$rows['apellido1']."</h1>
                    <h1 class='text-left'>".$rows['apellido2']."</h1>
                    <h1 class='text-left'>".$rows['facultad']."</h1>
                    <h1 class='text-left'>".$rows['telefono']."</h1>
          ...
          echo "<div class='row' style='margin-top: 1%;'>
                  <div class='col'>
                    <h1 class='text-left'>No hay profesores en el sistema</h1>
                  </div>
                </div>";
        }
        ...
        <div class="col-xl-8">
            <div class="row">
                <div class="col-md-4 col-lg-10 col-xl-12 text-center">
                    ...
                    <?php
                      $query = "SELECT * FROM UMA WHERE tipo LIKE 'PDI'";
                      $resultados = mysqli_query($conexion, $query);
                      while($rows = mysqli_fetch_assoc($resultados)){
                        echo "<div class='row' style='margin-top: 1%;'>";
                        echo "<div class='col' style='border: 1px solid #56c6c6; border-radius: 10px;'>";
                        echo "<p style='font-size: 20px;font-weight: bold;' class='text-left'><a  href='insertar_relacion_actividad_profesor.php?id=".$_GET['id']."&correo=".$rows['email']."     '>".$rows['apellido1']." ".$rows['apellido2'].", ".$rows['nombre']."</a></p>";                                           
                        ...
                        echo    
                                ...
                                <p class='text-left'>Telefono: ".$rows['telefono']."</p>
								<p class='text-left'>Asignaturas que imparte: </br>";
