    ...
    <?php
      function Search($input){
        ...
        if($count > 0){
          ...
          echo "<div class='row' style='margin-top: 1%;'>
                  <div class='col'>
                    <h1 class='text-left'>No hay actividades sin revisar</h1>
                  </div>
                </div>";
        }
        ...
        <div class="col-xl-8">
            <div class="row">
                <div class="col-md-4 col-lg-10 col-xl-12 text-center">
                    ...
                    <?php
                      $query = "SELECT * FROM Actividad WHERE aceptada = 0 AND (tipo = 'Docencia' OR tipo = 'Investigacion')";
                      $resultados = mysqli_query($conexion, $query);
                      while($rows = mysqli_fetch_assoc($resultados)){
                        echo "<div class='row' style='margin-top: 1%;'>";
                        echo "<div class='col' style='border: 1px solid #56c6c6; border-radius: 10px;'>";
                        echo "<p style='font-size: 20px;font-weight: bold;' class='text-left'><a  href='modificar_actividad.php?id=".$rows['ID']."'>".$rows['Titulo']."</a></p>";

                        if ($rows['tipo'] != "Sin Aceptar") {
